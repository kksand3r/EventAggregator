...
function DigitUnit({ value, label }: { value: number; label: string }) {
    return (
        <div className="flex flex-col items-center flex-1">
            <div className="w-full py-2 rounded-xl bg-white/60 border border-white flex items-center justify-center shadow-sm">
...
function CountdownCard({ event }: { event: EventListItem }) {
    ...
    return (
        <div className="group relative flex flex-col h-full bg-white/40 backdrop-blur-xl border border-white/60 rounded-[24px] p-5 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300">
            <div className="mb-4">
                <span
            ...
            <div className={`rounded-2xl p-4 mb-5 border transition-colors ${isUrgent ? 'bg-rose-50/50 border-rose-100' : 'bg-slate-50/50 border-slate-100'}`}>
                {isUrgent && (
                    <div className="flex items-center justify-center gap-1.5 mb-3">
                        <span className="w-1.5 h-1.5 rounded-full bg-rose-500 animate-pulse" />
                        <span className="text-[10px] font-black uppercase tracking-widest text-rose-500">Starting soon</span>
                    </div>
                )}
        ...
        <section className="mb-16">
            <div className="flex items-center gap-3 mb-8 px-2">
                ...
                <div>
                    ...
                    <p className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400">Live countdown to premiere</p>
                </div>
            </div>
