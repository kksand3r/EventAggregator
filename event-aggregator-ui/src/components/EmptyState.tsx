...
function IllustrationSearch() {
    return (
        <svg width="120" height="120" viewBox="0 0 120 120" fill="none"
            ...
            {/* Ripple waves */}
            <circle cx="52" cy="52" r="42" stroke="rgba(139,92,246,0.08)" strokeWidth="1"
                    strokeDasharray="4 4"/>
            <circle cx="52" cy="52" r="50" stroke="rgba(139,92,246,0.05)" strokeWidth="1"
                    strokeDasharray="3 5"/>
        </svg>
    );
}
...
export default function EmptyState({ type = "generic", query, onReset }: EmptyStateProps) {
    ...
    return (
        <div className="flex flex-col items-center justify-center py-20 px-6 text-center"
            ...
            {/* Text */}
            <h3 className="text-xl font-semibold mb-2" style={{ color: "#1e1a3a" }}>
                {title}
            </h3>

            {query && (
                <p className="text-sm mb-1" style={{ color: "rgba(30,26,58,0.55)" }}>
                    No results for{" "}
            ...
            <p className="text-sm max-w-xs" style={{ color: "rgba(30,26,58,0.45)" }}>
                {sub}
            </p>
            ...
            {onReset && (
                <button
                    onClick={onReset}
                    className="mt-6 px-5 py-2.5 rounded-lg text-sm font-semibold transition-all"
                    style={{
                    ...
                    onMouseEnter={e => {
                        (e.currentTarget as HTMLElement).style.background = "rgba(139,92,246,0.20)";
                    }}
                    onMouseLeave={e => {
