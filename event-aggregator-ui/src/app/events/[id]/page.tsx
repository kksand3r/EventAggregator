...
export default function EventDetailsPage() {
    ...
    useEffect(() => {
        ...
        let cancelled = false;
        fetchEvents({ page: 1, pageSize: 15 })
            .then((res) => {
                if (cancelled) return;
                const list = res.data.filter(e =>
                    e.id !== eventId && (e.city === event.city || e.category === event.category)
                );
        ...
        return (
            <div className="min-h-screen gradient-bg flex items-center justify-center">
                <div className="card-glass rounded-2xl p-8 text-center max-w-md">
                    ...
                    <button onClick={() => router.push('/')} className="inline-flex items-center gap-2 text-violet-600 font-medium hover:underline">
    ...
    return (
        <div className="min-h-screen gradient-bg">
            ...
            <main className="max-w-6xl mx-auto px-4 py-12 pb-24">
                <div className="card-glass rounded-[3rem] overflow-hidden shadow-2xl mb-16 flex flex-col border border-white/50 bg-white/20">

                    {/* Верхня частина: Фото + Головна інфо */}
                    <div className="flex flex-col md:flex-row">
                        <div className="relative w-full md:w-5/12 shrink-0 h-[500px] md:h-auto">
                            ...
                            <div className="absolute top-8 left-8 flex flex-col gap-3">
                                <span className="inline-flex items-center gap-1.5 rounded-full bg-white/95 backdrop-blur-md px-3 py-1.5 text-[11px] font-black text-violet-700 shadow-xl uppercase tracking-wider w-fit">
                                    <Eye className="h-3.5 w-3.5 shrink-0" /> 
                                    <span>{event.viewCount.toLocaleString("uk-UA")}</span>
                                </span>
                                
                                <span className="inline-flex items-center rounded-full bg-violet-600 px-3 py-1.5 text-[11px] font-black text-white shadow-xl uppercase tracking-widest w-fit">
                                    {event.category}
                                </span>
                            </div>
                        </div>
                        ...
                        <div className="flex-1 p-10 md:p-16 lg:p-20 flex flex-col justify-center relative bg-white/40">
                            <div className="relative z-10">
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-10 mb-12">
                                    ...
                                    <div className="flex items-center gap-5">
                                        <div className="p-4 bg-fuchsia-500 rounded-2xl text-white shadow-lg shadow-fuchsia-200">
                    ...
                    {/* Нижня частина: AI Summary */}
                    {aiSummary && (
                        <div className="p-10 md:p-20 bg-gradient-to-b from-white/50 to-white/20 border-t border-white/60">
                            <div className="max-w-4xl mx-auto">
                                <div className="relative group">
                                    <div className="absolute -inset-1 bg-gradient-to-r from-violet-400 to-fuchsia-400 rounded-[2rem] blur opacity-15 group-hover:opacity-25 transition duration-1000"></div>
                                    <div className="relative p-8 md:p-12 rounded-[2.5rem] bg-gradient-to-br from-violet-600 via-violet-700 to-indigo-800 shadow-2xl border border-white/10">
                                        <div className="flex items-center gap-3 mb-6">
                                            <div className="p-2 bg-white/20 rounded-lg backdrop-blur-sm">
                                                <Sparkles className="h-5 w-5 text-white" />
                                            </div>
                                            <h4 className="text-[11px] font-black uppercase tracking-[0.3em] text-violet-100/90">Чому варто піти (AI Analysis)</h4>
                                        </div>
