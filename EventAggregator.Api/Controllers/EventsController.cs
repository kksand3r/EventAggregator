...
namespace EventAggregator.Api.Controllers
{
    ...
    public class EventsController : ControllerBase
    {
            ...
            [FromQuery] int pageSize = 20)
        {
            ...
            // 🎯 ВАЖЛИВИЙ ФІКС: Шукаємо по чистому полю "city", а не "city.keyword"
            if (!string.IsNullOrWhiteSpace(city) && city != "All")
                filters.Add(f => f.Term(t => t.Field("city").Value(city.ToUpper())));

            // 🎯 ВАЖЛИВИЙ ФІКС: Шукаємо по вкладеному полю "category.keyword", як визначено в мапінгу Text поля
        ...
        {
            // 🎯 ВАЖЛИВИЙ ФІКС: Змінено "city.keyword" на "city" відповідно до схеми індексу
        ...
        {
            // 🎯 ВАЖЛИВИЙ ФІКС: Змінено "city.keyword" на "city"
            var response = await _client.SearchAsync<ScrapedEvent>(s => s.Index("events").Size(0).Aggregations(a => a
