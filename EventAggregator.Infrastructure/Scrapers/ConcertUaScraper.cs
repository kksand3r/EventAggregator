...
public class ConcertUaScraper : IEventScraper
{
    ...
    public async Task<List<ScrapedEvent>> ScrapeAsync(IBrowser browser)
    {
        ...
        _logger.LogInformation("🚀 Початок надшвидкого скрапінгу Concert.ua через API + JSON-LD...");
        ...
        var cityTasks = _citySlugs.Select(async citySlug =>
        {
            ...
            try
            {
                        ...
                        try
                        {
                            ...
                            foreach (var node in elementsToProcess)
                            {
                                ...
                                var newEvent = new ScrapedEvent
                                {
                                    ...
                                    // ОНОВЛЕНО: Тільки чистий малий слаг для повної синхронізації з EventsController
                                    City = cityLower,
                ...
                if (cityEventsCount > 0)
                    _logger.LogInformation("✅ Concert.ua (JSON-LD): Отримано {Count} подій для міста {City}", cityEventsCount, cityLower.ToUpper());
            }
            catch (Exception ex)
            {
        ...
        _logger.LogInformation("🏁 Concert.ua: Збір завершено. Всього знайдено унікальних подій: {Count}", allCollectedEvents.Count);
