...
1. Назви міст у базі зберігаються ВИКЛЮЧНО латиницею у верхньому регістрі.
   Використовуй ТІЛЬКИ ці значення:
   Київ → KYIV, Львів → LVIV, Одеса → ODESA, Харків → KHARKIV,
   Дніпро → DNIPRO, Івано-Франківськ → IVANO-FRANKIVSK,
   Вінниця → VINNYTSIA, Полтава → POLTAVA, Житомир → ZHYTOMYR,
   Запоріжжя → ZAPORIZHZHIA, Тернопіль → TERNOPIL,
   Чернівці → CHERNIVTSI, Чернігів → CHERNIHIV, Суми → SUMY,
   Хмельницький → KHMELNYTSKYI, Рівне → RIVNE, Луцьк → LUTSK,
   Миколаїв → MYKOLAIV, Ужгород → UZHHOROD, Кропивницький → KROPYVNYTSKYI.
   Кирилиця у полі city НЕ існує — term-фільтр буде провалений.
2. Категорії у базі ТІЛЬКИ: "concerts", "theatres", "stand-up", "child", "clubs", "festivals", "inshe".
   Концерт/концерти → concerts, Театр/вистава → theatres, Стендап → stand-up,
   Дітям/дитячий → child, Клуб → clubs, Фестиваль → festivals, інше → inshe.
